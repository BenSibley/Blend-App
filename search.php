<?php get_header(); ?>
    ...
    <div class="post-header search-box">
        <h1 class="post-title">
            ...
            if($total_results) {
                printf(__('%d search results for','blend-app'), $total_results);
            } else {
                _e("No search results for ", 'blend-app');
            }
            ?>
    ...
    if($total_results) {
        ...
        <div class="search-bottom search-box">
            <p><?php _e("Can't find what you're looking for?  Try refining your search:", "blend-app"); ?></p>
            <?php get_search_form(); ?>
        </div>
    <?php } ?>
<?php get_footer(); ?>